...
class TaskDetailViewController: UIViewController {
    ...
    @IBAction func didTapAttachPhotoButton(_ sender: Any) {
        // Check if we have access to the photo library
        if PHPhotoLibrary.authorizationStatus(for: .readWrite) != .authorized {
            ...
            // Show photo picker if already authorized
            presentImagePicker()
        }
    }
    
    @IBAction func didTapViewPhotoButton(_ sender: UIButton) {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "PhotoSegue" {
            if let photoViewController = segue.destination as? PhotoViewController {
                photoViewController.task = task  // ✅ Pass task object
            }
        }
    }
...
extension TaskDetailViewController: PHPickerViewControllerDelegate {
    ...
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        // Optionally, you could add code here to save the image to your Task model
